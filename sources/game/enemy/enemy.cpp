...
#define _EPS				(1E-11)		// 浮動小数点誤差(0.00000000001)
...
#ifdef _DEBUG
    m_pHitDome = CHitCheckSphere::Create(m_vPos, m_vSize);
#endif
    return S_OK;
}
...
// エネミーの視野内判定
//*************************************************************************************************
bool CEnemy::CheckEnemySite(void)
{
    ...
    while (scene != NULL) {
        if (scene->GetType() == SCENE_TYPE_PLAYER) {
            ...
            m_vSiteDir = -m_vDir;           //視野ベクトル
            m_vSiteDir.y = 0.0f;
            m_vTargetDir = m_vPos - playerPos;  //プレイヤーへの方向ベクトル
