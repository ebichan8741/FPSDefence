...
int CEnemyLoader::m_MaxEnemy = 0;

void CEnemyLoader::EnemyLoad(void)
{
    FILE *fp;
    ENEMYDATA data[100];
    char buf[100];
...
out:
    ...
    for (int i = 0; i <= m_MaxEnemy; i++) {
        ...
        case ENEMY_TYPE_FLYING:
            CGameMode::m_pEnemy.push_back(CEnemyFly::Create(data[i].pos, data[i].hp, ID));
            //CLifeBarFrame::Create(data[i].pos, ID);
            //CLifeBar::Create(data[i].pos, ID);
            ID++;
            break;

        case ENEMY_TYPE_WALKER:
            break;
...
int CEnemyLoader::GetMaxEnemy(void)
{
    return m_MaxEnemy;
}